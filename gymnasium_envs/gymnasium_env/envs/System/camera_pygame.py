...
class CameraPygame:
    def __init__(self, init_with_camera_on=False):
        pygame.init()
        self.window = pygame.display.set_mode((initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT))
        ...
        self.particle_locs = [[initializations.GUI_FRAME_WIDTH / 2, initializations.GUI_FRAME_HEIGHT / 2]]
        ...
        self.data_extractor = data_extractor.data_extractor(self.coil_locs, initializations.COIL_NAMES)
    ...
    def drawGrids(self, frame, frame_size):
        if self.flag_grid_on:
            cv2.line(frame, (0, int(initializations.GUI_FRAME_WIDTH / 2)), (frame_size, int(initializations.GUI_FRAME_WIDTH / 2)), initializations.GUI_GRID_AXIS_COLOR, initializations.GUI_GRID_AXIS_THK)
            cv2.line(frame, (int(initializations.GUI_FRAME_WIDTH / 2), 0), (int(initializations.GUI_FRAME_WIDTH / 2), frame_size), initializations.GUI_GRID_AXIS_COLOR, initializations.GUI_GRID_AXIS_THK)

            for i in range(1, self.grid_dim):
                offset = int(i * (int(initializations.GUI_FRAME_WIDTH / 2) / self.grid_dim))
                cv2.line(frame, (0, int(initializations.GUI_FRAME_WIDTH / 2) + offset), (frame_size, int(initializations.GUI_FRAME_WIDTH / 2) + offset), initializations.GUI_GRID_COLOR, initializations.GUI_GRID_THK)
                cv2.line(frame, (0, int(initializations.GUI_FRAME_WIDTH / 2) - offset), (frame_size, int(initializations.GUI_FRAME_WIDTH / 2) - offset), initializations.GUI_GRID_COLOR, initializations.GUI_GRID_THK)
                cv2.line(frame, (int(initializations.GUI_FRAME_WIDTH / 2) + offset, 0), (int(initializations.GUI_FRAME_WIDTH / 2) + offset, frame_size), initializations.GUI_GRID_COLOR, initializations.GUI_GRID_THK)
                cv2.line(frame, (int(initializations.GUI_FRAME_WIDTH / 2) - offset, 0), (int(initializations.GUI_FRAME_WIDTH / 2) - offset, frame_size), initializations.GUI_GRID_COLOR, initializations.GUI_GRID_THK)
    ...
    def getBlobData(self, frame):
        ...
        for particle_loc in self.particle_locs:
            particle_locs_img.append(functions.absolute_to_image(particle_loc[0], particle_loc[1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT))

        self.blob_lost, particle_locs_img, self.particle_rads = Blob_detect.iden_blob_detect(detection_frame, particle_locs_img, self.particle_rads)

        self.particle_locs = []
        for particle_loc_img in particle_locs_img:
            self.particle_locs.append(functions.image_to_absolute(particle_loc_img[0], particle_loc_img[1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT))
    ...
    def drawBlobs(self, frame, particle_locs):
        if self.flag_detecting_objects:
            for i in range(len(particle_locs)):
                particle_loc_img = functions.absolute_to_image(particle_locs[i][0], particle_locs[i][1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT)
                cv2.circle(frame, particle_loc_img, self.particle_rads[i], initializations.GUI_BLOB_COLOR, initializations.GUI_BLOB_THK)

    def drawSolenoids(self, frame, coil_locs, coil_vals):
        if self.flag_show_solenoid_state:
            for i in range(len(self.coil_locs)):
                coil_loc_img = functions.absolute_to_image(coil_locs[i][0], coil_locs[i][1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT)
                self.drawSolenoidState(frame, coil_vals[i], coil_loc_img[0], coil_loc_img[1])
        ...
        if self.flag_show_target_position:
            for i in range(len(goal_locs)):
                if not self.flag_setting_multiple_goals or i != 0:
                    goal_loc_img = functions.absolute_to_image(goal_locs[i][0], goal_locs[i][1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT)
                    cv2.line(frame, (goal_loc_img[0] + initializations.GUI_GOAL_SIZE, goal_loc_img[1] + initializations.GUI_GOAL_SIZE), (goal_loc_img[0] - initializations.GUI_GOAL_SIZE, goal_loc_img[1] - initializations.GUI_GOAL_SIZE), initializations.GUI_GOAL_COLOR, initializations.GUI_GOAL_THICKNESS)
                    cv2.line(frame, (goal_loc_img[0] + initializations.GUI_GOAL_SIZE, goal_loc_img[1] - initializations.GUI_GOAL_SIZE), (goal_loc_img[0] - initializations.GUI_GOAL_SIZE, goal_loc_img[1] + initializations.GUI_GOAL_SIZE), initializations.GUI_GOAL_COLOR, initializations.GUI_GOAL_THICKNESS)
    ...
    def drawGoalVector(self, frame, particle_loc, goal_locs):
        if self.flag_show_goal_vector and not self.flag_setting_multiple_goals:
            particle_loc_img = functions.absolute_to_image(particle_loc[0], particle_loc[1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT)
            goal_loc_img = functions.absolute_to_image(goal_locs[0][0], goal_locs[0][1], initializations.GUI_FRAME_WIDTH, initializations.GUI_FRAME_HEIGHT)
            cv2.line(frame, particle_loc_img, goal_loc_img, initializations.GUI_GOAL_VECTOR_COLOR, initializations.GUI_GOAL_VECTOR_THICKNESS)
    ...
    def process_frame(self):
        ...
        if initializations.VERBOSE:
            print("##################################################\nTime taken to process camera frame:", (time.time() - time_stamp) * 1000, " ms")
            time_stamp = time.time()
        ...
        if initializations.VERBOSE:
            print("Time taken to detect blob:", (time.time() - time_stamp) * 1000, " ms")
            time_stamp = time.time()

        if self.flag_gen_log_data:
            self.data_extractor.record_datapoint(self.particle_locs, self.coil_vals, self.goal_locs)

        self.drawElements(frame, self.particle_locs, self.goal_locs, self.coil_locs, self.coil_vals, initializations.GUI_FRAME_WIDTH)
        ...
        if initializations.VERBOSE:
            print("Time taken to draw screen elements and create log datapoint (if activated):", (time.time() - time_stamp) * 1000, " ms")
            time_stamp = time.time()
        ...
        if initializations.VERBOSE:
            print("Time taken to record video frame:", (time.time() - time_stamp) * 1000, " ms")
            time_stamp = time.time()
        ...
        if initializations.VERBOSE:
            print("Time taken to take snapshot:", (time.time() - time_stamp) * 1000, " ms")
        ...
        try:
            ...
            if coil_vals is None:
                if self.flag_automatic_control:
                    self.coil_vals = control_algorithm.get_coil_vals(self.particle_locs[0], self.goal_locs[0], self.coil_vals, self.coil_locs)

            else:
                self.coil_vals = functions.limit_coil_vals(coil_vals)
            ...
            d = functions.distance(self.particle_locs[0][0], self.particle_locs[0][1], self.goal_locs[0][0], self.goal_locs[0][1])
            ...
            if initializations.VERBOSE:
                print("##################################################\nTime taken to run video frame:", (time.time() - time_stamp) * 1000, " ms")
                time_stamp = time.time()

            if initializations.VERBOSE:
                print("Time taken to run control algorithm:", (time.time() - time_stamp) * 1000, " ms")
                time_stamp = time.time()

            if initializations.VERBOSE:
                print("Time taken to process events in Pygame:", (time.time() - time_stamp) * 1000, " ms\n##################################################\n")

            if initializations.VERBOSE:
                print("Total time taken to run one frame:", (time.time() - time_stamp_total) * 1000, " ms\n\n##################################################\n\n")
