...
class Simulator:
    ...
    def updateParticleLocation(
        ...
        Args:
            ...
            render : Checks if screen rendering is on
        ...
        if not flag_edit_particle_mouse:
            ...
            if self.framerate != None and render:
                functions.sleep(max(0, dt - (time.time() - self.prev_time)))
            self.prev_time = time.time()
...
if __name__ == "__main__":
    ...
    while True:
        particle_locs, goal_locs, coil_vals, coil_locs = sim.getState(True)
